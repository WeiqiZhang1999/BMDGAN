                ...
                if j > i:
                    fuse_layer.append(
                        nn.Sequential(
                            ...
                            LayerHelper.get_norm_layer(num_features=num_inchannels[i], norm_type=self.norm_type),
                        ...
                        if k == i - j - 1:
                            ...
                            conv3x3s.append(
                                nn.Sequential(
                                    ...
                                    # ),
                                    LayerHelper.get_norm_layer(num_features=num_inchannels[j],
...
class HighResolutionTransformer(nn.Module):
    ...
    def __init__(self,
        ...
        cfg = MODEL_CONFIGS[cfg]
        self.output_ncs = cfg.STAGE4.NUM_CHANNELS.copy()

        self.norm_type = norm_type
        self.padding_tyep = padding_type

        self.conv1 = nn.Conv2d(input_nc, 64, kernel_size=3, stride=2, padding=1, bias=False, padding_mode=padding_type)
        # self.bn1 = nn.BatchNorm2d(64, momentum=BN_MOMENTUM)
        self.bn1 = LayerHelper.get_norm_layer(num_features=64, norm_type=norm_type)
        self.conv2 = nn.Conv2d(64, 64, kernel_size=3, stride=2, padding=1, bias=False, padding_mode=padding_type)
    ...
    def _make_head(self, pre_stage_channels):
        ...
        for i in range(len(pre_stage_channels) - 1):
            ...
            downsamp_module = nn.Sequential(
                nn.Conv2d(
                    ...
                    padding=1,
                    groups=in_channels,
                    padding_mode=self.padding_tyep
                ),
                # nn.BatchNorm2d(in_channels, momentum=BN_MOMENTUM),
                LayerHelper.get_norm_layer(num_features=in_channels, norm_type=self.norm_type),
                nn.Conv2d(in_channels, out_channels, kernel_size=1, stride=1),
                # nn.BatchNorm2d(out_channels, momentum=BN_MOMENTUM),
                LayerHelper.get_norm_layer(num_features=out_channels, norm_type=self.norm_type),
        ...
        final_layer = nn.Sequential(
            ...
            LayerHelper.get_norm_layer(num_features=2048, norm_type=self.norm_type),
            nn.ReLU(inplace=True),
        )
        ...
        for i in range(num_branches_cur):
            if i < num_branches_pre:
                if num_channels_cur_layer[i] != num_channels_pre_layer[i]:
                    transition_layers.append(
                        nn.Sequential(
                            ...
                            # ),
                            LayerHelper.get_norm_layer(num_features=num_channels_cur_layer[i],
                                                       norm_type=self.norm_type),
                            nn.ReLU(inplace=True),
                        )
                    )
                ...
                for j in range(i + 1 - num_branches_pre):
                    ...
                    conv3x3s.append(
                        nn.Sequential(
                            nn.Conv2d(inchannels, outchannels, 3, 2, 1, bias=False, padding_mode=self.padding_tyep),
                            # nn.BatchNorm2d(outchannels, momentum=BN_MOMENTUM),
                            LayerHelper.get_norm_layer(num_features=outchannels, norm_type=self.norm_type),
                            nn.ReLU(inplace=True),
                        )
                    )
    ...
    def _make_layer(
        ...
        if stride != 1 or inplanes != planes * block.expansion:
            downsample = nn.Sequential(
                ...
                LayerHelper.get_norm_layer(num_features=planes * block.expansion, norm_type=self.norm_type)
            )
        layers = []
        ...
        if isinstance(block, GeneralTransformerBlock):
            ...
            layers.append(block(inplanes,
                                ...
                                downsample,
                                norm_type=self.norm_type,
                                padding_type=self.padding_tyep))

        inplanes = planes * block.expansion
        for i in range(1, blocks):
            layers.append(block(inplanes, planes, norm_type=self.norm_type, padding_type=self.padding_tyep))
...
def get_hrt_backbone(config: str,
                     input_nc: int,
                     drop_path_rate=None,
                     norm_type=None,
                     padidng_type="reflect",
                     dropout_rate=0.,
                     dropout_layer=nn.Dropout2d):

    return HighResolutionTransformer(cfg=MODEL_CONFIGS[config],
                                     input_nc=input_nc,
                                     drop_path_rate=drop_path_rate,
                                     norm_type=norm_type,
                                     padding_type=padidng_type,
                                     dropout_rate=dropout_rate,
                                     dropoput_layer=dropout_layer)
