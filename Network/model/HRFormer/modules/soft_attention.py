#!/usr/bin/env python
# -*- coding: utf-8 -*-
# @Time    : 2/27/2023 6:59 PM
# @Author  : ZHANG WEIQI
# @File    : soft_attention.py
# @Software: PyCharm
import torch
...
class SoftmaxFreeAttentionKernel(nn.Module):
    ...
    def forward(self, Q, V, H, W):
        ...
        if self.ratio == 1:
            ...
            X = torch.matmul(torch.matmul(kernel_1_, newton_inverse_kernel(kernel_2_, self.max_iter)),
                             torch.matmul(kernel_3_, V))
...
class SoftmaxFreeNormAttentionKernel(SoftmaxFreeAttentionKernel):
    def __init__(self, dim, num_heads, ratio, use_conv, max_iter=20, kernel_method="cuda"):
        super(SoftmaxFreeSymNormAttentionKernel, self).__init__(dim, num_heads, ratio, use_conv, max_iter,
                                                                kernel_method)
...
class SoftmaxFreeNormTransformer(SoftmaxFreeTransformer):
    def __init__(self, dim, num_heads, ratio, conv_size, drop_path=0., max_iter=20, kernel_method="torch"):
        super(SoftmaxFreeNormTransformer, self).__init__(dim, num_heads, ratio, conv_size, drop_path, max_iter,
                                                         kernel_method)
...
class SoftmaxFreeNormTransformerBlock(SoftmaxFreeTransformerBlock):
    def __init__(self, dim, num_heads, ratio, drop_path=0., conv_size=3, max_iter=20, kernel_method="cuda"):
        super(SoftmaxFreeNormTransformerBlock, self).__init__(dim, num_heads, ratio, drop_path, conv_size, max_iter,
                                                              kernel_method)
