...
from Network.model.basicsr.utils.registry import ARCH_REGISTRY

from Network.model.basicsr.archs.arch_util import to_2tuple, trunc_normal_
...
# for idynamic
from Network.model.basicsr.archs.dlgsanet_idynamicdwconv_util import *
...
if __name__ == '__main__':
    ...
    # model = DLGSANet(in_chans=1, dim=dim, upscale=upscale, groups=groups, blocks=blocks, window_size=window_size,
    #                  idynamic_num_heads=idynamic_num_heads, restormer_num_heads=restormer_num_heads,
    #                  upsampler='bmd', input_resolution=(height, width))
    encoder = HighResolutionTransformer(cfg='hrt_base')
    fuse = MultiscaleClassificationHead(input_nc=sum(encoder.output_ncs),
                                                         output_nc=(64 * (2 ** 2)),
                                                         norm_type="group",
                                                         padding_type="reflect")
    up = UpsamplerHead()
    model = nn.Sequential(encoder, fuse, up)
    ...
    dummy_input = torch.rand(1, 1, height, width).to(device)
