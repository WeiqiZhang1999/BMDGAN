#!/usr/bin/env python
# -*- coding: utf-8 -*-
# @Time    : 2/7/2023 2:04 PM
# @Author  : ZHANG WEIQI
# @File    : BMDFormerModel.py
# @Software: PyCharm
...
class BMDFormerModel(TrainingModelInt):
...
class BMDFormerModelInference(InferenceModelInt):
    ...
    def load_model(self, load_dir: AnyStr, prefix="ckp"):
        for signature in ["netG_up", "netG_fus", "netG_enc", "transformer"]:
            net = getattr(self, signature)
            load_path = str(OSHelper.path_join(load_dir, f"{prefix}_{signature}.pt"))
